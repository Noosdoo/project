...
def build_dataset_parallel(people_list_path=PEOPLE_LIST_FILE, dataset_path=DATASET_FILE,
    ...
    def process_person(name):
        try:
            ...
            # 基本情報
            rec = {"name": name, "summary": page.summary,
                   "full_text":page.text,"features": None, "wikidata": None}
            
            # 1. キーワードベース（静的）の特徴抽出
            features = extract_features_from_summary(clean_text(page.text))
            features_b=extract_features_from_summary(clean_text(page.summary))

            # 2. Janome（動的）の特徴を抽出し、featuresにマージする
            dynamic_features = extract_dynamic_features_from_summary(clean_text(page.text))
            dynamic_features_b=extract_dynamic_features_from_summary(clean_text(page.summary))
            
            # 動的特徴が空だった場合のログ
            if not dynamic_features and page.text:
                # summaryはあったのに、Janomeが特徴を返さなかった場合
                print(f"  [DEBUG-PROCESS] {name}: Summaryはありましたが、動的特徴は0個でした。")

            if dynamic_features:
                features.update(dynamic_features)
                features.update(dynamic_features_b)
